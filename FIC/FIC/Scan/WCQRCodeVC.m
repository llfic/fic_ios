...
- (void)setupQRCodeScan {
    ...
    [obtain setBlockWithQRCodeObtainScanResult:^(SGQRCodeObtain *obtain, NSString *result) {
        /**
         目前扫描得到的就是别人的publicKey，也就是地址
         扫描完成之后，就将结果返回给上一级控制器
         */
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(WCQRCodeVC:scanResult:)]) {
            [weakSelf.delegate WCQRCodeVC:weakSelf scanResult:result];
        }
        [weakSelf.navigationController popViewControllerAnimated:YES];
//        if (result) {
//            [ZMBProgressHUD SG_showMBProgressHUDWithModifyStyleMessage:@"正在处理..." toView:weakSelf.view];
//            [obtain stopRunning];
...
//                [ZMBProgressHUD SG_hideHUDForView:weakSelf.view];
...
- (void)rightBarButtonItenAction {
    ...
    [obtain setBlockWithQRCodeObtainAlbumResult:^(SGQRCodeObtain *obtain, NSString *result) {
        ...
        if (result == nil) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [ZMBProgressHUD SG_hideHUDForView:weakSelf.view];
                [ZMBProgressHUD SG_showMBProgressHUDWithOnlyMessage:@"未发现二维码/条形码" delayTime:1.0];
            });
        } else {
            ScanSuccessJumpVC *jumpVC = [[ScanSuccessJumpVC alloc] init];
            jumpVC.comeFromVC = ScanSuccessJumpComeFromWC;
