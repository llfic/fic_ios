...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"ZMBProgressHUD needs to be accessed on the main thread.");
...
@interface ZMBProgressHUDRoundedButton : UIButton
@end


@implementation ZMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ZMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ZMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (ZMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            ZMBProgressHUD *hud = (ZMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = ZMBProgressHUDAnimationFade;
    _mode = ZMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(ZMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == ZMBProgressHUDAnimationZoom) {
        type = animatingIn ? ZMBProgressHUDAnimationZoomIn : ZMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == ZMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == ZMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == ZMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == ZMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    id<ZMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    backgroundView.style = ZMBProgressHUDBackgroundStyleSolidColor;
    ...
    UIButton *button = [ZMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[ZMBRoundProgressView class]];

    ZMBProgressHUDMode mode = self.mode;
    ...
    else if (mode == ZMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[ZMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == ZMBProgressHUDModeDeterminate || mode == ZMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[ZMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == ZMBProgressHUDModeAnnularDeterminate) {
            [(ZMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == ZMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == ZMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[ZMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[ZMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[ZMBRoundProgressView class]]) {
        ZMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ZMBRoundProgressView appearanceWhenContainedIn:[ZMBProgressHUD class], nil];
#else
        appearance = [ZMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[ZMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((ZMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((ZMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[ZMBBarProgressView class]]) {
        ZMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ZMBBarProgressView appearanceWhenContainedIn:[ZMBProgressHUD class], nil];
#else
        appearance = [ZMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[ZMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((ZMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((ZMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
- (void)setMode:(ZMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == ZMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation ZMBRoundProgressView
...
@implementation ZMBBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = ZMBProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = ZMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(ZMBProgressHUDBackgroundStyle)style {
    if (style == ZMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = ZMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    ZMBProgressHUDBackgroundStyle style = self.style;
    if (style == ZMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == ZMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation ZMBProgressHUDRoundedButton
